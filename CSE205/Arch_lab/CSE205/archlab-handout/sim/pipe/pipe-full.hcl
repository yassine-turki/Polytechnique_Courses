...
# Descrition:
# - Fetch stage: 
# We need the instruction iaddq to be valid, so we add it in the "bool instr_valid" function. 
# Moreover, we will need a register (rB) and valC (since we have the immediate value V). It should be mentionned that rA is actually 'F', meaning no register.
# - Decode stage:
# We do not add IIADDQ into register rA since we have the immediate value, but we need rB so we add the function for srcB word. 
# We also do not need M since we will not modify the memory, but only the register rB. Therefore d_dstE is set to rB for IIADDQ.
# - Execute stage:
# The ALU will need valC as input A and valB as input B. We also need iiaddq to update the Condition Codes so we add an "or" statement in the function "bool set_cc "
# - Memory stage:
# Nothing is happening in this stage so we do not modify anything
# - Write back stage:
# The register rB is updated with valE computed by the ALU.	
# - PC update:
# We will use the default value, so we do not modify the code.

...
class GameGrid:
    ...
    def neighbors(self, i, j):
        ...
        for k in range(j-1, j+2):
            if i-1 >=0 and i-1 <= self.nrows-1 and k >=0 and k <= (self.ncols-1) :
                l.append((i-1,k))
            if i+1 >=0 and i+1 <= self.nrows-1 and k >=0 and k <= self.ncols-1:
                l.append((i+1,k))
            if i >=0 and i <= self.nrows-1 and k >=0 and k <= self.ncols-1 and k!=j:
                l.append((i,k))
        return l
    ...
    def flags_nearby(self, i, j):
        ...
        for n in neighbors:
             (x,y)=n
             if self.square_at(x, y).flagged ==True:
                      num+=1
        return num
    ...
    def place_mine(self, i, j):
        ...
        """
        m=self.square_at(i, j)
        if m.mined==False:
            m.mined=True
            l=self.neighbors(i, j)
            for neighbour in l :
                (x,y)=neighbour
                a= self.square_at(x,y)
                a.mines_nearby+=1
            self.nmines+=1
    ...
    def reveal(self, i, j):
        ...
        """
        s=self.square_at(i, j)
        if s.revealed==True or s.flagged==True:
            return None
        else:
            s.revealed=True
            self.squares_revealed+=1
            if s.mined==False and s.mines_nearby==0:
                l=self.neighbors(i, j)
                for k in l:
                    (x,y)=k
                    self.reveal(x,y)
    ...
    def chording(self, i, j):
        ...
        """
        s=self.square_at(i, j)
        if s.mines_nearby==self.flags_nearby(i,j):
             l=self.neighbors(i, j)
             for k in l:
                (x,y)=k
                self.reveal(x,y)
    ...
    def random_game(self, n_mines):
        ...
        if n_mines <= 0 or n_mines >= self.nrows*self.ncols:
            ...
            self.reset()
            c=[(x,y) for x in range(0,self.nrows) for y in range(0,self.ncols)]
            if n_mines> len(c):
                for i in c:
                    self.place_mine(i[0], i[1])
            else:
                for m in random.sample(c,n_mines):
                    self.place_mine(m[0], m[1])
...
class MinesweeperApp:
    ...
    def update_mine_counter(self):
        ...
        d=self.game.nmines-self.game.nflags
        self.mine_counter_str.set(str(d))
    ...
    def update_time_counter(self):
        ...
        time=self.game_time()
        self.time_counter_str.set(str(time))
    ...
    def left_click_handler(self, i, j):
        ...
        self.draw_board()
        if self.game.game_lost != False:
            self.game_over(False)
        if self.game.game_won != False:
            self.game_over(True)
    ...
    def right_click_handler(self, i, j):
        ...
        """
        s1=self.game.square_at(i,j)
        s2=self.game.square_at(i,j)
        if s1.revealed != True: 
            if s2.flagged != True:
                s1.flagged = True
                self.game.nflags +=1

...
move =>p.
destruct p.
+apply (ins_elem1 (insertion_sort l) n n0).
apply hl.
trivial.
+
rewrite H.
apply (ins_elem2 (insertion_sort l) n).
Qed.
...
elim => [|n l hl] n0//=.
move => p.
destruct (ins_elem3 (insertion_sort l) n n0);trivial.
right;trivial.
left.
apply hl.
trivial.
...
(* Yes because we have proved an if and only if lemma *)
...
Fixpoint to_list (t : tree) :=
  ...
  match t with
  | Leaf => nil _
  | Node m t1 t2 => cons m (merge (to_list t1) (to_list t2))
  end.
...
(* The complexity of the new function is O(nlog(n)) *)

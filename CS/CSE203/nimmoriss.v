...
- by rewrite !Monoid.simpm => lt; rewrite divn_small.
...
by case: ltnP => // gei; rewrite /bit nth_default ? Monoid.simpm.
Qed.
...
- rewrite bit_oversize //= divn_small ?mod0n //.
  ...
  - apply: leq_sum; case=> /= j ltj _.
    by case: b.[_]; rewrite Monoid.simpm.
...
- rewrite big_mkord; apply: leq_sum => /= -[/= k ltk] _.
  ...
  by rewrite /F; case: b.[k]; rewrite Monoid.simpm.
...
rewrite !(@b2nWE s) /=; try by rewrite leq_max leqnn Monoid.simpm.
...
destruct e as (p, e).
destruct e as (q, e).
destruct e as (H1, H2, H3).
rewrite /weight H2 H3.
rewrite weight_rD.
rewrite weight_rD.
rewrite weight_rS.
rewrite weight_rS.
rewrite (bxorC (n2b (s2 i)) (weight_r q)).
rewrite (bxorC (n2b (s1 i)) (weight_r q)).
rewrite bxorA.
rewrite bxorA.
rewrite -bxorA.
rewrite -bxorA.
rewrite (bxorC (n2b (s1 i)) (n2b (s2 i))).
rewrite -bxorA.
rewrite (bxorC (n2b (s1 i)) (n2b (s2 i) .+ n2b (s1 i))).
rewrite -bxorA.
rewrite -bxorA.
rewrite bxorbb.
rewrite bxorb0.
...
(* FIXME *)
  ...
  rewrite ws2.
  move => sum0.
  move : (bxorb0 (n2b (s1 i))).
  rewrite -(bxorbb (n2b (s2 i))).
  rewrite bxorA.
  rewrite sum0.
  rewrite bxor0b.
  move => eqs.
  move : (n2b_inj (s2 i) (s1 i) (eqs)).
  move => eq.
  case h => ineq random.
  move : ineq.
  rewrite eq.
...
Lemma abstract_rows (s: seq nat) (i: nat):  (weight_r s).[i] -> exists (n : nat), 
  (n2b (nth 0 s n)).[i] /\ n < (size s).
Proof.
  induction s.
  - simpl.
    rewrite b0E.
  ...
  - simpl.
    rewrite bxorE.
    destruct (n2b a).[i] eqn:ali ; simpl.
    + move => _.
      exists 0.
      simpl.
      split.
...
Lemma exists_good_row(s: state) : weight s <> 0%:B -> exists (i : 'I_p),
  (n2b (s i)).[(size (weight s)).-1].
Proof.
  rewrite hibit_neq0W.
  move : ((size (weight s)).-1) => li.
  unfold weight.
  move => wrs.
  move : (abstract_rows wrs).
  move => l.
  destruct l.
  destruct H.
  move : H0.
  ...
  exists (Ordinal ltxp).
  move : H.
  rewrite (nth_rows s (Ordinal ltxp)).
  trivial.
Qed.

Lemma ltlte (a b : nat): a.-1 < b -> a <= b.  
Proof.
  induction b; induction a => //=.
Qed.

Lemma ineqbits (x y : bits) : y <> 0%:B -> size(y) <= size(x) -> x.[(size y).-1] = true -> (b2n (x.+y)) < (b2n x).
Proof.
  move => neq0 ltxy xbiton. 
  Search b2n.
  apply lt_n2b.
  exists (size y).-1.
  - move => l.
...
Lemma bits_not_oversize (b:bits) (i:nat) : b.[i] -> i < size b.
Proof.
  move => bi.
  have : (size b <= i -> ~~b.[i]).
  - move => ltsbi.
    move : (bit_oversize ltsbi).
    rewrite bi.
    done.
  rewrite bi.
  simpl.
  destruct (i < size b)  eqn:ltib.
  - trivial.
  - move : (ltnNge i (size b)) => rev_ineq. 
    move : ltib.
    rewrite rev_ineq.
...
Proof.
  ...
  move : (rows s) => r.
  move => ws0.
  destruct (exists_good_row ws0).
  exists x.
  remember (fun (i: 'I_p) => (if i == x then (b2n (n2b (s x).+weight_r (rows s))) else (s i))) as s'.
  exists s'.
  ...
  rewrite Heqs'.
  rewrite (eq_refl x).
  have : b2n (n2b (s x) .+ weight_r (rows s)) < (b2n (n2b (s x))).
  apply ineqbits.
  apply ws0.
  move : (bits_not_oversize H) => swz.
  apply ltlte.
  apply swz.
  ...
  rewrite Heqs'.
  move => j neqj.
  destruct (j == x) eqn:beqj.
  move : neqj.
  simpl.
  done.

  trivial.
  (* Finish Proving R x s s'*)

  move => rxss.
  split.
  rewrite (turn_weight rxss). 
  rewrite Heqs'.
  rewrite (eq_refl x).
  rewrite b2nK.
  unfold weight.
  rewrite (bxorC (n2b (s x)) (weight_r (rows s))).
  apply bxorbb.
  apply rxss.
Qed.
End Nim.
